...
from .IFEval.evaluation_main import (InputExample, test_instruction_following_loose,
...
class IFEval(BaseEval):
    ...
    def score(self, p_samples: List[Dict], p_res_path: str):
        ...
        for t_smp in p_samples:
            ...
            results['strict'].append(if_strict)
            results['loose'].append(if_loose)

            t_smp['strict'] = sum(if_strict.follow_instruction_list) / len(if_strict.instruction_id_list)
            t_smp['loose'] = sum(if_loose.follow_instruction_list) / len(if_loose.instruction_id_list)
            t_smp['score'] = t_smp['strict']
        ...
        final_scores['score'] = (final_scores['Prompt-level-loose-accuracy'] + final_scores['Inst-level-strict-accuracy']) / 2
